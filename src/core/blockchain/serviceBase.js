...
class Transaction {
    ...
    constructor(addressFrom, addressTo, amount, type)
    {
...
class BlockchainServiceBase {
    ...
    async createState(address)                  { throw new Error('not implemented'); }
    async updateState(address, newBalance, tx)  { throw new Error('not implemented'); }
    async getState(address)                     { throw new Error('not implemented'); }
    async getStatesCount()                      { throw new Error('not implemented'); }
    
    async getHeight()                           { throw new Error('not implemented'); }
    
    async addPendingTx(tx)                      { throw new Error('not implemented'); }
    async getPendingTxs()                       { throw new Error('not implemented'); }
    ...
    async generateNewAddress() {
        try {
            ...
            const xpub = "xpub6AHA9hZDN11k2ijHMeS5QqHx2KP9aMBRhTDqANMnwVtdyw2TDYRmF8PjpvwUFcL1Et8Hj59S3gTSMcUQ5gAqTz3Wd8EsMTmF3DChhqPQBnU"
    ...
    async send(addressFrom, addressTo, amount) {
        ...
        if (!stateFrom.balance || stateFrom.balance < amount)
            return false;
    ...
    async executeTx(/*Transaction*/ tx)
    {
        try {
            ...
            if (tx.type === TxType.txNormal) {
                // verify from address - address should exist
                const stateFrom = await this.getState(tx.addressFrom)
                if (!stateFrom)
                    return false;
                
                // check balance addressFrom
                if (!stateFrom.balance || stateFrom.balance < tx.amount)
                    return false;
            }
            ...
            if (!stateTo) 
                stateTo = await this.createState(tx.addressTo)
